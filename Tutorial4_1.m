...
% alpha_m = (10E5*(-V_m-0.045))/(exp(100*(-V_m - 0.045))-1);
% beta_m = 4*10E3*exp((-V_m - 0.070)/0.018);
% alpha_h = 70*exp(50*(-V_m-0.070));
% beta_h = (10E3)/(1+exp(100*(-V_m - 0.040)));
% alpha_n = 10E4*(-V_m - 0.060)/(exp(100*(-V_m-0.060))-1);
% beta_n = 125*exp((-V_m - 0.070)/0.08);

% m_std = alpha_m/(alpha_m + beta_m);
% h_std = alpha_h/(alpha_h + beta_h);
% n_std = alpha_n/(alpha_n + beta_n);

dt = 0.02E-4; t_max = 0.35;
t = 0:dt:t_max;

I_0 = 0;
I_app = ones(size(t))*I_0;
% I_app(floor(0.1/dt):floor(0.2/dt)) = 0.22E-9;
...
%Cannot show subthreshold oscillation
